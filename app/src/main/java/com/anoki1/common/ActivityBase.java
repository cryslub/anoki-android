...
import com.anoki1.etc.MessageListActivity;
...
public class ActivityBase extends ActionBarActivity {
        ...
        popupMenu.setOnMenuItemClickListener(new PopupMenu.OnMenuItemClickListener() {
            public boolean onMenuItemClick(MenuItem item) {
                switch (item.getItemId()) {
                    ...
                    break;
                    case R.id.delete: {
                        Util.backGroundRest("prayer", "DELETE", prayer, new CallBack<Void>() {
                            @Override
    ...
    protected void setPrayerListPage(int id, final CallBack<User> callback) {
        setPrayerListPage(id, false, new CallBack<User>() {
            @Override
    ...
    protected void setPrayerListPage(int id,final boolean editable, final CallBack<User> callback){
        ...
        Util.backGroundRest("user/detail", "POST", user, User.class, new CallBack<User>() {
            @Override
            public void success(User user) {
                ...
                Type listType = new TypeToken<ArrayList<Prayer>>() {
                }.getType();
    ...
    public String setProfilePicture( Intent data){
        return Util.upload(data.getData(), getContentResolver(), new PictureCallBack() {
            @Override
    ...
    protected <T> void  backGroundRest(final String path,final String method,final Object in) {

        Util.backGroundRest(path, method, in, new CallBack<Void>() {
            @Override
            public void success(Void result) {
    ...
    protected <T> void  backGroundRest(final String path,final String method,final Object in, final Type classOfT, final CallBack<T> callback) {

        Util.backGroundRest(path,method,in,classOfT,callback);
