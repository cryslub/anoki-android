...
public class DBManager  extends SQLiteOpenHelper {
    ...
    public void insertAlarm(Alarm alarm){
        ...
        db.execSQL("INSERT INTO ALARM (USER,TYPE,NAME1,NAME2,TIME,PICTURE,GID,CHECKED) VALUES(" + alarm.user + ",'" + alarm.type + "','" + alarm.name1 + "','" + alarm.name2 + "','" + alarm.time + "'," + alarm.picture + "," + alarm.gId + ",0)");
        db.close();
    }
    ...
    public Prayer getOneRequest(int id){
        SQLiteDatabase db = getReadableDatabase();

        List<Prayer> list = new ArrayList<Prayer>();
        Cursor cursor = db.rawQuery("SELECT PRAYER,CHECKED FROM REQUEST WHERE PRAYER = " + id, null);
        if (cursor.moveToNext()) {
